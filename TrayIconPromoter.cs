...
/// Taskbar → Other system tray icons. For a tray-only app like MicMute that
...
/// dead paths, accumulating "N MicMutes" cruft in the Settings list.
///
...
internal static class TrayIconPromoter
{
    ...
    /// "N MicMutes in tray-overflow Settings" cruft that .NET single-file
        ...
        try
        {
            ...
            foreach (var subName in subNames)
            {
                try
                {
                    ...
                    Log.Info($"TrayIconPromoter.SweepStaleEntries: removed {subName} -> {path}");
                    swept++;
                }
                catch (Exception ex)
                {
                    Log.Warn($"TrayIconPromoter.SweepStaleEntries: subkey {subName}: {ex.Message}");
                }
            }
        }
        catch (Exception ex)
        {
            Log.Warn($"TrayIconPromoter.SweepStaleEntries: {ex.Message}");
        }

        if (swept > 0)
            Log.Info($"TrayIconPromoter.SweepStaleEntries: removed {swept} stale entr{(swept == 1 ? "y" : "ies")} for {ourExeName}");
        return swept;
    }
    ...
    internal static HashSet<string> CaptureBaseline()
    {
        ...
        {
            Log.Warn($"TrayIconPromoter.CaptureBaseline: {ex.Message}");
            return null;
        }
    }
    ...
    internal static bool TryPromote(string exePath, HashSet<string> baseline)
    {
            ...
            foreach (var subName in root.GetSubKeyNames())
            {
                try
                {
                    ...
                    if (!string.IsNullOrEmpty(path))
                    {
                        ...
                        {
                            Log.Info($"TrayIconPromoter: {subName} IsPromoted=0 -- respecting user's choice");
                            continue;
                        }
                        if (current is int i1 && i1 == 1) continue;

                        sub.SetValue("IsPromoted", 1, RegistryValueKind.DWord);
                        Log.Info($"TrayIconPromoter: promoted {subName} for {path}");
                        continue;
                    }
                ...
                {
                    Log.Warn($"TrayIconPromoter: subkey {subName}: {ex.Message}");
                }
            }
            ...
            if (!identified && orphanCandidates.Count == 1)
            {
                ...
                try
                {
                    ...
                    if (sub is not null)
                    {
                        ...
                        if (current is int i0 && i0 == 0)
                        {
                            Log.Info($"TrayIconPromoter: orphan {subName} IsPromoted=0 -- respecting user's choice");
                            identified = true;
                        }
                        else
                        {
                            ...
                            sub.SetValue("IsPromoted", 1, RegistryValueKind.DWord);
                            Log.Info($"TrayIconPromoter: claimed orphan {subName} -> wrote ExecutablePath + IsPromoted=1 for {exePath}");
                            identified = true;
                        }
                    }
                }
                ...
                {
                    Log.Warn($"TrayIconPromoter: claim orphan {subName}: {ex.Message}");
                }
            }
            else if (orphanCandidates.Count > 1)
            {
                Log.Info($"TrayIconPromoter: {orphanCandidates.Count} new orphan subkeys -- deferring to next tick");
            }
        ...
        catch (Exception ex)
        {
            ...
            Log.Warn($"TrayIconPromoter: {ex.Message}");
            return false;
        }
    }
}
